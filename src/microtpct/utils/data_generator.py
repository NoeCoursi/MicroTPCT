...
# Parameter validation and config utilities
...
def validate_parameters(
    *,
    ...
    if match_fraction + quasi_fraction > 1.0:
        raise ValueError("match_fraction + quasi_fraction must be <= 1.0")

    if redundancy_rate > 0 and mutation_rate == 0:
        raise ValueError("mutation_rate must be > 0 when redundancy_rate > 0")
...
# Main public API (NO GROUND TRUTH)
...
def generate_benchmark_databases(
    ...
    return target_db, query_db, config
...
if __name__ == "__main__":
    ...
    target_db, query_db, _ = generate_benchmark_databases(
        n_proteins=20,
